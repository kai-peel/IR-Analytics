...
def main():
       ...
       dbconnection.dbcursor.execute("select distinct uesid from uescodes where uesid between 253440 and 288947 and length(encodedbinary) < 2") 
...
#and b.activeflag = 'Y' and encodedbinary is null and exists ( select 1 from uespulses d where d.uesid = c.uesid) and c.activeflag ='Y' """)
       timestampsuffix=strftime("%Y%m%d%H%M%S")
       logfilename="fplog_%s.txt" %(timestampsuffix)
       errfilename="fperrorfilelog_%s.txt" %(timestampsuffix)
...
def clearcache():
    serverlist = [
    "ec2-46-137-236-66.ap-southeast-1.compute.amazonaws.com","ec2-54-251-198-154.ap-southeast-1.compute.amazonaws.com","ec2-54-251-79-243.ap-southeast-1.compute.amazonaws.com","ec2-54-251-227-148.ap-southeast-1.compute.amazonaws.com"]
