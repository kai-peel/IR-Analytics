...
def test_codeset(log, irdb, irgen, codeset):
    try:
        ...
        for ea in rows:
            ...
            log.write('%d|%s|%s|%d|%s|%d|%d\n' % (ea[0], fmt, ea[2], ea[3], ea[4], frequency, repeat_count))
            log.write(','.join(map(str, main_frame)))
            log.write('\nrepeat:\n')
            log.write(','.join(map(str, repeat_frame)))
            log.write('\ntoggle:\n')
            log.write(','.join(map(str, toggle_frame)))
            log.write('\n')
    except Exception, e:
        print e
...
def test():
    f = ir.Logger('hydra', 1)
    v2 = ir.DBConnection()
    hy = gen.Hydra(host=IRDBSG)

    start_time = datetime.datetime.now()
    print "Started: %s." % start_time

    test_codeset(f.logs[0], v2, hy, 200006)
